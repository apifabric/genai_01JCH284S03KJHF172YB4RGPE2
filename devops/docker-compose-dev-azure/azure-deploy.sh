...
resourcename="petshop"  # lower case, only
resourcegroup="petshop_rg"
...
# git clone https://github.com/apilogicserver/petshop.git
# cd petshop
...
az group create --name petshop_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group petshop_rg --sku S1 --is-linux
az container create --resource-group petshop_rg --name petshop --image apilogicserver/petshop:latest --dns-name-label petshop --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//petshop.westus.azurecontainer.io:5656'
