...
def declare_logic():
    ...
    # Updates the customer's balance based on open orders.
    Rule.sum(derive=Customer.balance, as_sum_of=Order.amount_total, where=lambda row: row.date_shipped is None)

    # Calculates the total amount of an order from its details.
    Rule.sum(derive=Order.amount_total, as_sum_of=OrderDetail.amount)

    # Calculates the amount for each order detail line.
    Rule.formula(derive=OrderDetail.amount, as_expression=lambda row: row.quantity * row.unit_price)

    # Copies the unit price from Product to Item.
    Rule.copy(derive=OrderDetail.unit_price, from_parent=Product.price)

    # Ensure customer balance does not exceed credit limit for Check Credit requirement.
